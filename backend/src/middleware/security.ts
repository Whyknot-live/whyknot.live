...
export const securityHeaders = createMiddleware(async (c, next) => {
   ...
   securityLogger.debug('Applying security headers', { path: c.req.path })
...
export function rateLimitMiddleware(options: {
   ...
   return createMiddleware(async (c, next) => {
      ...
      securityLogger.debug('Rate limit check', { ip, path: c.req.path, keyPrefix })
      ...
      if (process.env.REDIS_URL) {
         try {
            ...
            if (!rateLimit.allowed) {
               securityLogger.warn('Rate limit exceeded (Redis)', { ip, path: c.req.path, keyPrefix })
               return c.json({
                  error: 'rate_limited',
      ...
      if (!handledByRedis) {
         ...
         if (!fallback.allowed) {
            securityLogger.warn('Rate limit exceeded (in-memory)', { ip, path: c.req.path, keyPrefix })
            return c.json({
               error: 'rate_limited',
               message: 'Too many requests, please try again later',
